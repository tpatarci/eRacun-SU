/**
 * IBAN Validator Service - Main Entry Point
 *
...
app.post('/api/v1/validate', (req: Request, res: Response) => {
  ...
  try {
    const result: IBANValidationResult = validateIBAN(oib);
...
app.post('/api/v1/validate/batch', (req: Request, res: Response) => {
  ...
  try {
    const results: IBANValidationResult[] = validateIBANBatch(oibs);
    return res.json({
...
server.listen(PORT, HOST, async () => {
  logger.info(`IBAN Validator service listening on ${HOST}:${PORT}`);

/**
 * RabbitMQ Consumer for OIB Validation
 *
...
import { validateOIB, validateOIBBatch, type OIBValidationResult } from '../oib-validator.js';
import { recordValidation, recordBatchValidation, recordError } from '../observability/metrics.js';

const QUEUE_NAME = 'oib-validation-requests';
const RESULT_QUEUE_NAME = 'oib-validation-results';
...
interface ValidationResponse {
  requestId: string;
  result?: OIBValidationResult;
  results?: OIBValidationResult[];
  error?: string;
  timestamp: string;
}
...
export async function setupRabbitMQ(url: string, logger: Logger): Promise<void> {
  try {
    ...
    await channel.bindQueue(QUEUE_NAME, EXCHANGE_NAME, 'oib.validation.request');
    await channel.bindQueue(RESULT_QUEUE_NAME, EXCHANGE_NAME, 'oib.validation.result');
...
async function handleMessage(msg: amqp.ConsumeMessage, channel: amqp.Channel, logger: Logger): Promise<void> {
  ...
  try {
    ...
    // Single OIB validation
    if (request.oib) {
      const result = validateOIB(request.oib);
    ...
    // Batch validation
    else if (request.oibs && Array.isArray(request.oibs)) {
      const results = validateOIBBatch(request.oibs);
      recordBatchValidation(request.oibs.length, 'rabbitmq');
    ...
    // Invalid request
      ...
      response = {
        requestId: request.requestId,
        error: 'Invalid request: must provide either "oib" or "oibs"',
        timestamp: new Date().toISOString(),
      };
